...
<body style="height: auto">
	<%
		ArrayList<Formular_request> listOD = Dao.getListFormula_Request();
			...
			<div id="search">
				...
				<div id="select-quan">
					<form action="Getfooddistrict">
						Hiển thị: <select id="FR_show">
							<option value="all" selected="selected">Tất cả đơn hàng</option>
							<option value="Chưa kiểm tra" selected="selected">Chưa kiểm tra</option>
		...
		<div id="content"
					...
					<%
						...
						for (int i = val_start; i < val_end - tmp; i++) {
							...
							out.print("<tr style='height: 10'><td>" + (i + 1) + "</td><td style='text-align: left'>"
									+ Formular_request.showFormulaRequest(listOD.get(i)));
			...
			<div id="page_devider"
				...
				<div
					...
					<span>Trang: </span>
					<%
						for (int i = 0; i < (listOD.size() / 10); i++) {
							out.print("<a style='padding-right: 7px' href='Admin-formula-request.jsp?page=" + (i + 1) + "'>"
									+ (i + 1) + "</a>");
						}
					%>
				</div>
						...
						function() {	
							...
							$('.btnstatus_select')
									.on(
											'click',
											function() {
												window.location = 'Update_formula_request?id_fr='
														+ $('.tmp_id_fr').val()
														+ '&status='
														+ $('#status_select')
																.val();
											...
											function() {
												var choice = confirm('Chắc chắn xóa? '
														+ $(this)
																...
																.text());
												if (choice === true) {
													window.location = 'Delete_formula_request?id_FR='
															+ $(this).closest(
																	'tr').find(
																	'td:eq(4)')
																	.text();
